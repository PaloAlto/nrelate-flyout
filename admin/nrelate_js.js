...
if ( typeof nRelate == 'undefined' ) {
	...
	nRelate = function() {		
		...
		function mDialog( sDialogId )
		{
			...
			dcont.style.width = args.width + 'px';
		...
		{
			if( _nrelate.xWidth( this.dialog ) > _nrelate.xClientWidth() || _nrelate.xHeight( this.dialog ) > _nrelate.xClientHeight() ) {
				this.closebtn.parentNode.removeChild( this.closebtn	 );
				var  myWindow = window.open("data:text/html," + encodeURIComponent(this.dialog.innerHTML), "_blank" );
				myWindow.focus();
				return true;
			}
