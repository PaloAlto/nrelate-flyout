...
0;d.style.left="-9999px";d.style.width=c.width+"px";d.style.padding="30px 10px 5px";d.style.backgroundColor="#fff";d.style.border="solid 1px #333";c.height&&(d.style.height=c.height+"px");var e=document.createElement("img");e.src="http://static.nrelate.com/common_js/close_window.gif";e.title="Close dialog";e.style.cursor="pointer";e.style.position="absolute";e.style.top="5px";e.style.right="5px";f.bind(e,"click",function(b){n.instances[a].hide();f.is_function(c.callback)&&c.callback(!1)});d.appendChild(e);
d.appendChild(this.dialog.parentNode.removeChild(this.dialog));if("confirm"==c.type){var g,h,m;g=document.createElement("span");f.xac(g,"nr_dialog_btn nr_btn_ok");g.innerHTML=c.txt_ok||"Ok";f.bind(g,"click",function(){n.instances[a].hide();f.is_function(c.callback)&&c.callback(!0)});h=document.createElement("span");f.xac(h,"nr_dialog_btn nr_btn_cancel");h.innerHTML=c.txt_ok||"Cancel";f.bind(h,"click",function(){n.instances[a].hide();f.is_function(c.callback)&&c.callback(!1)});m=document.createElement("div");
f.xac(m,"nr_dialog_btns");m.appendChild(g);m.appendChild(h);d.appendChild(m)}this.closebtn=e;this.dialog=d;document.body.appendChild(this.dialog);d=n.grey;d||(d=document.createElement("div"),d.className="nr_dialog_overlay",d.style.position="fixed",d.style.overflow="hidden",d.style.zIndex="9000",d.style.opacity=0.3,d.style.filter="alpha(opacity=30)",d.style.backgroundColor="#000",n.grey=document.body.appendChild(d),f.bind(d,"click",function(){for(var a in n.instances)n.instances[a].hide();f.is_function(c.callback)&&
c.callback(!1)}));f.bind(window,"resize",function(){for(var a in n.instances)n.instances[a].is_opened&&n.instances[a].center()})}var f={init:function(a){var b,c=null;if(this.is_initialized)return this.debug("nRelate already setup"),!1;this.is_initialized=!0;this.extend(this.options,a,this.constants);if(this.is_object(this.options.plugins))for(b in this.options.plugins)this.is_defined(this.plugin_helpers[b])&&(c=c||{},c[b]=this.extend(!0,{},this.plugin_helpers[b]._defaults,this.options.plugins[b],
this.plugin_helpers[b]));this.options.plugins=c;this.plugins_sanitized=!0;for(b in this.options.plugins)this.snt(b);this.lcs();this.debug("Options initialized %o",this.options)},snt:function(a){var b,c,d,e=["cssstyle","thumbsize","ad_place","widgetstyle"];c={style:"cssstyle",thumb_size:"thumbsize"};for(b in c)this.is_defined(this.options.plugins[a][b])&&(this.options.plugins[a][c[b]]=this.options.plugins[a][b],delete this.options.plugins[a][b]);b=0;for(c=e.length;b<c;b++)d="supported_"+e[b]+"s",0>
this.in_array(this.options.plugins[a][e[b]],this.options.plugins[a][d])&&(this.options.plugins[a][e[b]]=this.options.plugins[a][d][0]);this.is_function(this.options.plugins[a].csnt)&&this.options.plugins[a].csnt.call(this,this.options.plugins[a])},lcs:function(){var a=this.options.debug_mode?this.options.debug_cs_api_url:this.options.cs_api_url,a=a+(this.to_slug(this.options.domain)+".js");this.debug("Loading custom scripts from %o",a);this.lr(a,"nrelate-widget-cs");var b=this;setTimeout(function(){b.acs({})},
5E3)},acs:function(a){if(this.plugins_sanitized&&!this.custom_scripts_loaded){this.custom_scripts_loaded=!0;var b;this.debug("Applying custom scripts");for(b in this.options.plugins)this.extend(this.options.plugins[b],a[b]);this.nr_cs_load_time=(new Date).getTime();if(!this.options.browser.msie)try{this.pd()}catch(c){}this.bdr(this.pd,"pd()")}},pd:function(){var a,b,c,d,e;for(p in this.options.plugins){this.is_function(this.options.plugins[p].fph)?(a=this.options.plugins[p].fph,this.debug("Use custom fph for %s",
p)):(a=this.dfph,this.debug("Use default fph for %s",p));this.is_function(this.options.plugins[p].fpl)?(b=this.options.plugins[p].fpl,this.debug("Use custom fpl for %s",p)):(b=this.dfpl,this.debug("Use default fpl for %s",p));this.is_function(this.options.plugins[p].fkw)?(c=this.options.plugins[p].fkw,this.debug("Use custom fkw for %s",p)):(c=this.dfkw,this.debug("Use default fkw for %s",p));this.debug("Parsing document for %s",p);d=this.options.plugins[p].cts.length;this.options.plugins[p].phs=a.call(this,
p);this.options.plugins[p].pls=b.call(this,p);e={};a=0;for(b=this.options.plugins[p].pls.length;a<b;a++)e[this.options.plugins[p].pls[a]]=1;this.is_home_page=Boolean(this.is_defined(window.nr_is_home)?window.nr_is_home:1<this.get_keys(e).length);this.options.plugins[p].kws=c.call(this,p);a=d;for(b=this.options.plugins[p].phs.length;a<b&&a<(1>=b?1:15);++a)this.act(a,p);this.mac()}},dfph:function(a){var b,c,d,e,g,h,m,l=[],f="post-body article-content entry-content entry post-entry post-inner postmeta2 post_content text postcontent single post-header content post art-PostContent jump-link entrytitle wrap blogPost post-footer postfooter commentpage post-footer-line".split(" "),
n=["post-footer","postfooter","commentpage","post-footer-line"],q=["format_teaser","sideContent","ngg-widget"];if(l=this.xgebi("nrelate_"+a+"_placeholder"))return this.debug("Backwards compatibility: using #nrelate_%s_placeholder: %o",a,l),[l];if(0<(l=this.xgebcn("nr_"+a+"_placeholder","div")).length)return this.debug("Using manually located .nr_%s_placeholder: %o",a,l),l;if(this.options.manual_ph_only)return[];b=0;for(e=f.length;b<e;b++)if(m=this.xgebcn(f[b]),0<(g=m.length)){this.debug("Found candidates with .%s selector",
f[b]);for(c=0;c<g;c++){d=0;for(h=q.length;d<h;d++)if(this.xhc(m[c],q[d])){this.debug("%o excluded because of class .%s",m[c],q[d]);m[c]=!1;break}m[c]&&0<=this.in_array(f[b],n)&&(this.is_defined(m[c].previousSibling)&&1===m[c].previousSibling.nodeType?(this.debug("Asigning previousSibling because of .%s",f[b]),m[c]=m[c].previousSibling):(this.debug("%o excluded because it doesn't have valid previousSibling",m[c]),m[c]=!1));m[c]&&(l[l.length]=m[c])}if(l.length)return this.debug("Document parsed. Found these placeholders %o",
l),l}else this.debug("No placeholders found with .%s selector",f[b]);if(l=this.xgebi("nrelate_"+a+"_backup_placeholder"))return this.debug("Backwards compatibility: using #nrelate_%s_backup_placeholder: %o",a,l),[l];this.debug("No placeholders found");return[]},dfpl:function(a){var b,c,d,e,g,h,m,l=[],f=["entry-title","post-title","entry-header"],n=["h3","h2","h1"];if(1==this.options.plugins[a].phs.length&&"undefined"!=typeof window.nr_pageurl)return this.debug("Manually defined nr_pageurl"),[{href:window.nr_pageurl}];
if(0<this.options.plugins[a].phs.length&&this.options.plugins[a].phs[0].getAttribute("data-permalink")){b=0;for(e=this.options.plugins[a].phs.length;b<e;b++)if(g=this.get_data_attr(this.options.plugins[a].phs[b],"permalink"))l[l.length]={href:g};else{l=[];break}if(l.length)return this.debug("Found data-permalink attribute on placeholders"),l}if(0<(m=this.xgeba("meta","property","og:url")).length){b=0;for(e=this.options.plugins[a].phs.length;b<e;b++)l[l.length]={href:m[0].content};this.debug("Found og:url meta tag");
return l}if(0<(m=this.xgeba("link","rel","canonical")).length){b=0;for(e=this.options.plugins[a].phs.length;b<e;b++)l[l.length]=m[0];this.debug("Found canonical link tag");return l}if(1<(l=this.xgeba("a","title","permanent link")).length)return this.debug("Found title='permanent link' permalinks %o",l),l;b=0;for(e=f.length;b<e;b++)for(c=0,g=n.length;c<g;c++){if(0<(h=this.xgebcn(f[b],n[c])).length)for(d=0;d<h.length;d++)0<(m=this.xgebtn("a",h[d])).length&&(l[l.length]=m[0]);if(0<l.length)return this.debug("Found permalinks on %s.%s %o",
n[c],f[b],l),l}if(0<(l=this.xgeba("a","rel","bookmark")).length)return this.debug("Found rel='bookmark' permalinks %o",l),l;if(0<(l=this.xgebcn("permalink","a")).length)return this.debug("Found class='permalink' permalinks %o",l),l;if(0<(c=this.xgebcn("post")).length){b=0;for(e=c.length;b<e;b++)0<(m=this.xgebtn("a",c[b])).length&&(l[l.length]=m[0]);if(0<l.length)return this.debug("Found first links inside div.post %o",l),l}if(0<(c=this.options.plugins[a].phs).length){b=0;for(e=c.length;b<e;b++)0<
(m=this.xgebtn("a",c[b])).length&&(l[l.length]=m[0]);if(0<l.length)return this.debug("Found first links inside div.post %o",l),l}this.debug("No permalinks found");return[]},dfkw:function(a){var b,c,d,e,g,h,m=document.title,l=[],f=["entry-title","post-title","entry-header"],n=["h3","h2","h1"];if("undefined"!=typeof window.nr_keywords&&"encodeURI(document.title)"!=window.nr_keywords)return this.debug("Manually defined nr_keywords"),[window.nr_keywords];if(0<this.options.plugins[a].phs.length&&this.options.plugins[a].phs[0].getAttribute("data-title")){b=
0;for(c=this.options.plugins[a].phs.length;b<c;b++)l[l.length]=this.get_data_attr(this.options.plugins[a].phs[b],"title");this.debug("Found data-title attribute on placeholders");return l}b=0;for(c=f.length;b<c;b++)for(d=0,e=n.length;d<e;d++){if(0<(g=this.xgebcn(f[b],n[d])).length)for(k=0;k<g.length;k++)if(h=g[k].innerText||g[k].textContent,this.is_home_page)l[l.length]=h;else if(0<=m.search(this.regex_escape(h))){b=0;for(c=this.options.plugins[a].pls.length;b<c;b++)l[b]=h;break}if(0<l.length)return this.debug("Found keywords on %s.%s %o",
n[d],f[b],l),l}if(!this.is_home_page){b=0;for(c=this.options.plugins[a].pls.length;b<c;b++)l[b]=m;this.debug("Single post page. Using document.title as keyword");return l}this.debug("No keywords found");return[]},act:function(a,b,c){var d;c="string"!=typeof c?"":c;d=document.createElement("div");d.id="nrelate_"+b+"_"+a;d.innerHTML=c;this.xac(d,"nrelate");this.xac(d,"nrelate_"+b);this.xac(d,"nr_clear");d.nr_api_called=!1;d.nr_plugin=b;d.nr_viewed=!1;this.options.plugins[b].phs[a].appendChild(d);this.options.plugins[b].cts[a]=
d},mac:function(){var a,b,c,d,e,g;for(a in this.options.plugins)if(this.is_function(this.options.plugins[a].should_load)&&!this.options.plugins[a].should_load.call(this,this.options.plugins[a]))this.debug("Ignore %s API calls because of should_load",a);else for(this.debug("Making API calls for %s",a),b=0,c=this.options.plugins[a].cts.length;b<c;b++)if(!(d=this.options.plugins[a].cts[b]).nr_api_called){if(!this.is_defined(this.options.plugins[a].pls[b]))if(1==c)this.options.plugins[a].pls[b]={href:window.location.href};
else continue;e=1==this.options.plugins[a].phs.length?document.title:"";e=this.aurlp(this.options.plugins[a].api_url,{plugin:this.options.plugins[a].shortname,tag:"nrelate_"+a,domain:this.options.domain,keywords:this.options.plugins[a].kws[b]||e,url:this.options.plugins[a].pls[b].href,nr_div_number:b,pr_id:this.get_print_id()});"referrer"in document&&document.referrer&&(e=this.aurlp(e,{referrer:document.referrer}));this.is_home_page||(d.nr_src_url=this.options.plugins[a].pls[b].href);e=this.apip(this.options.plugins[a],
e,d);if(g=this.gurlp(window.location.search,"predefined_set_override"))e=this.aurlp(e,{predefined_set_override:g});this.nr_parse_time=(new Date).getTime();if("performance"in window){g=this.nr_load_time-window.performance.timing.responseStart;var h=this.nr_cs_load_time-this.nr_load_time,m=this.nr_parse_time-Math.max(window.performance.timing.domInteractive,this.nr_cs_load_time);this.debug("#%s Loading time: %f | CS Loading time: %f | Parse time %f",d.id,g/1E3,h/1E3,m/1E3);e=this.aurlp(e,{loading_time:g,
...
a.widgetstyle=c.widgetstyle||a.widgetstyle||a.supported_widgetstyles[1];a.whats_this_link=c.whats_this_link||a.whats_this_link||!1;a.widget_id=c.widget_id||a.widget_id||"0";a.page_type_id=c.page_type_id||a.page_type_id||"0";a.page_type=c.page_type||a.page_type||"0";a.geo=c.geo||a.geo||"0";a.article_id=c.article_id||a.article_id||"0";if(a.cssstyle)if("custom"==a.cssstyle){c=this.to_slug(this.options.domain);var d=(this.options.debug_mode?this.options.debug_css_api_url:this.options.css_api_url)+c+".css",
d=this.aurlp(d,{v:a.cssversion});this.debug("Loading custom style from %o",d);this.lr(d,"nrelate-"+c+"custom-style",{type:"css",callback:this.fah})}else"none"!=a.cssstyle&&this.lr(this.options.default_stylesheet_url,"nrelate-styles",{type:"css",callback:this.fah});this.bind(window,"load",function(){nRelate.fah()});this.xac(b,"nrelate_"+a.cssstyle);0===a.widgetstyle?this.xac(b,"nr_text"):this.xac(b,"nr_"+a.thumbsize);"Separate"==a.ad_place?this.xac(b,"nr_2col"):this.xac(b,"nr_1row")},sct:function(a,
b,c){var d=this;this.xac(b,"nr_initializing");if(c.widget_id||a.widget_id)this.xac(b,"widget_id_"+(c.widget_id||a.widget_id)),b.nr_widget_id=c.widget_id||a.widget_id;b.nr_cols_num=c.cols_num||0;this.fh(b);this.ctr(b);this.aa(b);this.pr(b);a.whats_this_link&&this.wtl(b,a,c);c.uc&&this.ucs(b,a,c);this.is_function(a.csct)&&a.csct.call(this,a,b,c);this.xrc(b,"nr_initializing");this.xac(b,"nr_setup");this.bvc();this.bdr(function(){d.cab(b)})},fh:function(a){if((a=this.xgebi(a))&&!0!=a.nr_fixing_height){var b;
if(0==(b=this.xgebcn("nr_panel","a",a)).length){if(0<(b=this.xgebcn("nr_link","a",a)).length)for(c=0;c<b.length;c++)this.xac(b[c].parentNode,"nr_li_"+(c+1)),this.xac(b[c],"nr_row_"+(c+1))}else{a.nr_fixing_height=!0;var c,d,e,g,h;num_rows=num_cols=img_height=link_height=link_y=d=0;row_links=[];tallest=0;_col_counter=_row_counter=1;h=this.xgebtn("img",a);c=0;for(e=h.length;c<e;c++)img_height=Math.max(img_height,this.xHeight(h[c]));this.xrc(a,"rotate");c=0;for(e=b.length;c<e;c++)b[c].style.minHeight=
0,b[c].style.transition="none";d=this.xPageY(b[0]);c=0;for(e=b.length;c<e;c++)if(h=b[c],link_height=Math.max(this.xHeight(h),img_height),a.nr_cols_num)row_links[row_links.length]=h,tallest=tallest<link_height?link_height:tallest,num_cols=a.nr_cols_num;else{link_y=this.xPageY(h);if(d!=link_y){d=0;for(g=row_links.length;d<g&&!this.xhc(a,"nr_text");d++)row_links[d].style.minHeight=tallest+"px";row_links=[];d=link_y;tallest=link_height;row_links[row_links.length]=h}else row_links[row_links.length]=h,
tallest=tallest<link_height?link_height:tallest;num_cols=num_cols<row_links.length?row_links.length:num_cols}d=0;for(g=row_links.length;d<g&&!this.xhc(a,"nr_text");d++)row_links[d].style.minHeight=tallest+"px";c=0;for(e=b.length;c<e;c++)b[c].style.transition="";num_rows=b.length%num_cols+1;c=0;for(e=b.length;c<e;c++)h=b[c],this.xrc(h,"nr_odd_row|nr_even_row|nr_odd_col|nr_even_col|nr_first_col|nr_last_col|nr_first_row|nr_last_row|nr_row_[\\d]+|nr_col_[\\d]+|nr_unit_[\\d]+"),this.xac(h,"nr_unit_"+(c+
1)),this.xac(h,"nr_col_"+_col_counter),this.xac(h,"nr_row_"+_row_counter),1==_col_counter?this.xac(h,"nr_first_col"):_col_counter==num_cols&&this.xac(h,"nr_last_col"),1==_row_counter?this.xac(h,"nr_first_row"):_row_counter==num_rows&&this.xac(h,"nr_last_row"),this.xac(h,0==_row_counter%2?"nr_even_row":"nr_odd_row"),this.xac(h,0==_col_counter%2?"nr_even_col":"nr_odd_col"),_col_counter++,_col_counter>num_cols&&(_col_counter=1,_row_counter++);this.xhc(a,"nrelate_pol")&&this.xac(a,"rotate");a.nr_fixing_height=
!1;if(!this.is_dom_ready){var m=this;clearInterval(a.nr_fixheight_interval);a.nr_fixheight_interval=setTimeout(function(){m.fh(a)},100)}}}},fah:function(){var a,b;for(a in this.options.plugins)for(b=0;b<this.options.plugins[a].cts.length;b++)this.debug("Fix height of #%s",this.options.plugins[a].cts[b].id),this.fh(this.options.plugins[a].cts[b])},ctr:function(a){if(a=this.xgebi(a)){var b,c,d,e,g=this;if(0!=(d=this.xgebcn("nr_link","a",a)).length)for(b=0,c=d.length;b<c;b++)e=a.nr_plugin,this.bind(d[b],
"click",function(b){g.hclk(b,e,a)})}},hclk:function(a,b,c){a||(a=window.event);for(var d,e=this,g=this.evtsrc(a);"a"!=g.nodeName.toLowerCase();)g=g.parentNode;if(this.xhc(g,"nr_partner")||this.xhc(g,"nr_tx_link")&&this.xhc(g,"nr_external"))return!0;d=this.xhc(g,"nr_external")?"external":"internal";this.cwv(c,this.options.plugins[b]);b=this.aurlp(this.options.tracking_url,{plugin:this.options.plugins[b].shortname,type:d,domain:this.options.domain,src_url:c.nr_src_url||window.location.href,dest_url:this.clicked_link=
g.href,widget_id:c.nr_widget_id||this.options.plugins[b].widget_id,page_type_id:this.options.plugins[b].page_type_id,page_type:this.options.plugins[b].page_type,geo:this.options.plugins[b].geo,article_id:this.options.plugins[b].article_id,pr_id:this.get_print_id()});"referrer"in document&&document.referrer&&(b=this.aurlp(b,{referrer:document.referrer}));if(c=this.parse_json(this.get_data_attr(g,"tracking-params")))b=this.aurlp(b,c);if(c=this.parse_json(this.get_data_attr(g,"link-params")))this.clicked_link=
this.aurlp(this.clicked_link,c);this.middle_click=2==a.which||a.ctrlKey||a.metaKey||"_blank"==g.target?!0:!1;ifr=document.createElement("iframe");ifr.id="nr_clickthrough_frame_"+Math.ceil(100*Math.random());ifr.frameBorder="0";ifr.allowTransparency="true";ifr.style.height="0px";ifr.style.width="0px";ifr.style.display="none";this.bind(ifr,"load",function(){e.clicked_link&&!e.middle_click&&(window.location.href=e.clicked_link);g.nr_original_link&&(g.href=g.nr_original_link,g.nr_original_link=null);
...
this.xClientWidth();for(a in this.options.plugins)if(this.is_array(this.options.plugins[a].cts))for(c=0,d=this.options.plugins[a].cts.length;c<d;c++)b=this.options.plugins[a].cts[c],b.nr_count_views&&!b.nr_viewed&&(b.nr_top_distance=this.xPageY(b)+0.9*this.xHeight(b),b.nr_left_distance=this.xPageX(b)+0.9*this.xWidth(b),e>=b.nr_top_distance&&g>=b.nr_left_distance&&this.cwv(b,this.options.plugins[a]))},cwv:function(a,b){var c;a.nr_count_views&&!a.nr_viewed&&(a.nr_viewed=!0,this.xac(a,"nr_viewed"),c=
this.aurlp(this.options.views_url,{plugin:b.shortname,domain:this.options.domain,url:a.nr_src_url||window.location.href,widget_id:b.widget_id,page_type_id:b.page_type_id,page_type:b.page_type,geo:b.geo,pr_id:this.get_print_id(),top:a.nr_top_distance,left:a.nr_left_distance,api_time:a.nr_widget_initialized_time-a.nr_api_call_time,page_size:this.xDocSize()}),a.nr_all_units_blocked?c=this.aurlp(c,{all_units_blocked:1}):a.nr_all_partners_blocked&&(c=this.aurlp(c,{all_partners_blocked:1})),this.lr(c,a.id+
"_view"))},aa:function(a){if((a=this.xgebi(a))&&0!=(a=this.xgebcn("nr_sponsored","span",a)).length){var b,c,d=this,e=new r;b=0;for(c=a.length;b<c;b++)this.bind(a[b],"mouseover",function(a){a=d.evtsrc(a);e.css(a,"left",0,150,5)}),this.bind(a[b],"mouseout",function(a){a=d.evtsrc(a);var b=d.xWidth(a.parentNode)-18;e.css(a,"left",b,150,5)})}},pr:function(a){var b,c,d,e,g=this;if(0!=(e=this.xgebcn("nr_remove","span",a)).length)for(b=0,c=e.length;b<c;b++)d=e[b],this.bind(d,"click",function(b){var c,d,e,
f;d=g.evtsrc(b);e=d.parentNode;c=g.parse_json(g.get_data_attr(e,"tracking-params")).nrid;confirm("Are you sure to remove this unit?\n\nYou can undo this later on partners.nrelate.com")&&(f=g.xhc(e,"nr_partner")?"ad":g.xhc(e,"nr_external")?"external":"internal",c=g.aurlp(g.options.remove_url,{nrid:c,domain:g.options.domain,ct_id:a.id,nr_type:f,pr_id:g.get_print_id()}),g.lr(c),e.removeChild(d),d=new r,d.opacity(e,0.2,500,5));g.prevent_default(b)})},prc:function(a,b){var c,d;if(!(a=this.xgebi(a)))return!1;
b&&(c=new n(b),c.show());if(c=this.xgebi(a.id+"-api_call"))d=c.parentNode,d.removeChild(c);a.nr_api_called=!1;this.lr(this.aurlp(a.nr_api_url,{reload_uc:1}),a.id+"-api_call")},wtl:function(a,b,c){var d,e,g=this;if(0==this.xgebcn("nr_link","a",a).length)return!1;e=document.createElement("div");e.style.fontSize="0.7em";e.style.color="#AAA";e.style.textAlign="right";e.style.clear="both";d=document.createElement("span");d.innerHTML=c.whats_this_content||this.options.whats_this_content||b.whats_this_content;
d.title="about these links";d.style.cursor="pointer";this.xac(d,"nr_about");e.appendChild(d);a.appendChild(e);this.bind(d,"click",function(a){var d,e=b.fullname+"_whats_this_dialog";g.xgebi(e)||(d=document.createElement("iframe"),d.id=e,d.src=c.whats_this_url||b.whats_this_url,d.style.width="100%",d.style.height="260px",d.style.border="0px",d.setAttribute("border","0"),document.body.appendChild(d));d={};c.whats_this_width&&(d.width=c.whats_this_width);c.whats_this_height&&(d.height=c.whats_this_height);
(new n(e,d)).show();g.prevent_default(a)})},ucs:function(a,b,c){var d,e,g,h,f=this;if(!c.did||0==(h=this.xgebcn("nr_link","a",a)).length)return!1;this.tuc(a,b,c);b=0;for(d=h.length;b<d;b++)e=document.createElement("span"),this.xac(e,"nr_uc_btns"),g=document.createElement("span"),this.xac(g,"uc_btn"),g.innerHTML="&#x2715;",e.appendChild(g),h[b].appendChild(e),this.bind(g,"click",function(b){var d,e,g,h,n,q;f.prevent_default(b);h=f.xgebcn("nr_uc_dialog","*",a);n=0;for(q=h.length;n<q;n++)f.xrc(h[n].parentNode.firstChild,
"reduce_opacity"),f.xrc(h[n].parentNode.firstChild.nextSibling,"reduce_opacity"),h[n].parentNode.removeChild(h[n]);d=f.evtsrc(b).parentNode.parentNode;e=f.parse_json(f.get_data_attr(d,"tracking-params")).nrid;h=document.createElement("div");f.xac(h,"nr_uc_dialog");f.xac(d.firstChild,"reduce_opacity");f.xac(d.firstChild.nextSibling,"reduce_opacity");h.innerHTML='<p>Tell us why?</p><ul class="nr-uc-hide-options dropdown-menu"><li><a rel="1">Unrelated</a></li><li><a rel="2">Offensive</a></li><li><a rel="3">Too &ldquo;salesy&rdquo;</a></li><li><a rel="4">Repetitive</a></li><li><a rel="5">Other</a></li><li class="divider"></li><li class="nr-uc-dont-hide"><a rel="0">Don&rsquo;t hide it</a></li></ul>';
...
h.setTime(h.getTime()+864E5*c)}document.cookie=a+"="+escape(b)+(c?"; expires="+h.toGMTString():"")+"; path="+(d||"/")+(e?"; domain="+e:"")+(f?"; secure":"")},del:function(a,b,c){this.set(a,"",-1,b,c)}};n.prototype.show=function(){if(f.xWidth(this.dialog)>f.xClientWidth()||f.xHeight(this.dialog)>f.xClientHeight())return this.closebtn.parentNode.removeChild(this.closebtn),window.open("data:text/html,"+encodeURIComponent(this.dialog.innerHTML),"_blank").focus(),!0;var a=n.grey;this.is_opened=!0;a&&(this.dialog.greyZIndex=
...
Math.max(0,f.xScrollTop()+(b-this.dialog.offsetHeight)/2)+"px";c&&(c.style.left="0px",c.style.top="0px",c.style.width=a+"px",c.style.height=b+"px")};n.prototype.hide=function(a){var b=n.grey;this.is_opened=!1;b&&(a||(b.style.left="-10px",b.style.top="-10px",b.style.width="10px",b.style.height="10px"),this.dialog&&(b.style.zIndex=this.dialog.greyZIndex,this.dialog.style.left=-this.dialog.offsetWidth+"px",this.dialog.style.top="0px"))};n.grey=null;n.instances={};return{setup:function(){return f.init.apply(f,
Array.prototype.slice.call(arguments))},bind_dom_ready:function(){return f.bdr.apply(f,Array.prototype.slice.call(arguments))},acs:function(){return f.acs.apply(f,Array.prototype.slice.call(arguments))},sw:function(){return f.sw.apply(f,Array.prototype.slice.call(arguments))},prc:function(){return f.prc.apply(f,Array.prototype.slice.call(arguments))},fah:function(){return f.fah.apply(f,Array.prototype.slice.call(arguments))},auto_invoke:f.options.auto_invoke,xgebi:function(){return f.xgebi.apply(f,
Array.prototype.slice.call(arguments))},xgebcn:function(){return f.xgebcn.apply(f,Array.prototype.slice.call(arguments))},xgeba:function(){return f.xgeba.apply(f,Array.prototype.slice.call(arguments))},xgebtn:function(){return f.xgebtn.apply(f,Array.prototype.slice.call(arguments))},xrc:function(){return f.xrc.apply(f,Array.prototype.slice.call(arguments))},parse_json:function(){return f.parse_json.apply(f,Array.prototype.slice.call(arguments))},get_data_attr:function(){return f.get_data_attr.apply(f,
Array.prototype.slice.call(arguments))},lr:function(){return f.lr.apply(f,Array.prototype.slice.call(arguments))},debug:function(){return f.debug.apply(f,Array.prototype.slice.call(arguments))},cookie:s}}();var _comscore=_comscore||[],comscore_src=("https:"==document.location.protocol?"https://sb":"http://b")+".scorecardresearch.com/beacon.js";_comscore.push({c1:"7",c2:"16479758",c3:"1"});nRelate.lr(comscore_src,"nrelate-tracking",{async:!1});if(nRelate.auto_invoke){var script_tag,inline_options;
(script_tag=nRelate.xgebi("nrelate_loader_script"))&&(inline_options=nRelate.parse_json(nRelate.get_data_attr(script_tag,"nrelate-options")))?nRelate.debug("Inline options found: %o",inline_options):inline_options={};nRelate.setup(inline_options)}nRelate.bind_dom_ready()};
